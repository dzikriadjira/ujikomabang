...
@section('title', 'Edit Fasilitas - SMKN 4 Bogor')
                ...
                <form action="{{ route('admin.fasilitas.update', $fasilitas) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select id="color" name="color" required
                                ...
                                <option value="green" {{ old('color', $fasilitas->color) == 'green' ? 'selected' : '' }}>Hijau</option>
                                <option value="purple" {{ old('color', $fasilitas->color) == 'purple' ? 'selected' : '' }}>Ungu</option>
                                <option value="yellow" {{ old('color', $fasilitas->color) == 'yellow' ? 'selected' : '' }}>Kuning</option>
                                <option value="gray" {{ old('color', $fasilitas->color) == 'gray' ? 'selected' : '' }}>Abu-abu</option>
                                <option value="orange" {{ old('color', $fasilitas->color) == 'orange' ? 'selected' : '' }}>Orange</option>
                            </select>
                            @error('color')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select id="category" name="category" required
                                ...
                                <option value="otomotif" {{ old('category', $fasilitas->category) == 'otomotif' ? 'selected' : '' }}>Teknik Otomotif</option>
                                <option value="pemesinan" {{ old('category', $fasilitas->category) == 'pemesinan' ? 'selected' : '' }}>Teknik Pemesinan</option>
                                <option value="general" {{ old('category', $fasilitas->category) == 'general' ? 'selected' : '' }}>Umum</option>
                            </select>
                            @error('category')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <!-- Active -->
                        <div class="flex items-center">
                            <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $fasilitas->is_active) ? 'checked' : '' }}
                    ...
                    <div class="mt-6">
                        ...
                        <div id="features-container">
                            @foreach(old('features', $fasilitas->features ?? []) as $index => $feature)
