...
@section('title', 'Edit Jurusan - SMKN 4 Bogor')
    ...
    <main class="py-6">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white shadow-lg rounded-lg p-6 md:p-8">
                <h1 class="text-3xl font-bold text-gray-900 mb-6 text-center">Edit Jurusan</h1>
                ...
                <form action="{{ route('admin.jurusan.update', $jurusan) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" id="name" name="name" value="{{ old('name', $jurusan->name) }}" required
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" id="full_name" name="full_name" value="{{ old('full_name', $jurusan->full_name) }}" required
                                class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 @error('full_name') border-red-500 @enderror">
                            @error('full_name')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea id="description" name="description" rows="4" required
                                class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 @error('description') border-red-500 @enderror">{{ old('description', $jurusan->description) }}</textarea>
                            @error('description')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select id="color" name="color" required
                                ...
                                <option value="blue" {{ old('color', $jurusan->color) == 'blue' ? 'selected' : '' }}>Biru</option>
                                <option value="red" {{ old('color', $jurusan->color) == 'red' ? 'selected' : '' }}>Merah</option>
                                <option value="green" {{ old('color', $jurusan->color) == 'green' ? 'selected' : '' }}>Hijau</option>
                                <option value="purple" {{ old('color', $jurusan->color) == 'purple' ? 'selected' : '' }}>Ungu</option>
                                <option value="yellow" {{ old('color', $jurusan->color) == 'yellow' ? 'selected' : '' }}>Kuning</option>
                                <option value="gray" {{ old('color', $jurusan->color) == 'gray' ? 'selected' : '' }}>Abu-abu</option>
                            </select>
                            @error('color')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="text" id="icon" name="icon" value="{{ old('icon', $jurusan->icon) }}" required
                        ...
                        <div>
                            ...
                            <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', $jurusan->sort_order) }}" min="0"
                        ...
                        <!-- Featured -->
                        <div class="flex items-center">
                            <input type="checkbox" id="is_featured" name="is_featured" value="1" {{ old('is_featured', $jurusan->is_featured) ? 'checked' : '' }}
                                class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                            <label for="is_featured" class="ml-2 block text-sm text-gray-900">Tandai sebagai Jurusan Unggulan</label>
                        </div>
                    </div>

                    <!-- Kompetensi -->
                    <div class="mt-6">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Kompetensi</label>
                        <div id="competencies-container">
                            @foreach(old('competencies', $jurusan->competencies ?? []) as $index => $competency)
                            <div class="flex items-center space-x-2 mb-2">
                                <input type="text" name="competencies[]" value="{{ $competency }}" required
                                ...
                                @else
                                    <button type="button" onclick="addCompetency()" class="bg-green-500 text-white px-3 py-2 rounded-md hover:bg-green-600">
                    ...
                    <!-- Prospek Karir -->
                    <div class="mt-6">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Prospek Karir</label>
                        <div id="careers-container">
                            @foreach(old('careers', $jurusan->careers ?? []) as $index => $career)
                            <div class="flex items-center space-x-2 mb-2">
                                <input type="text" name="careers[]" value="{{ $career }}" required
                                    class="flex-1 px-4 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
                                ...
                                @else
                                    <button type="button" onclick="addCareer()" class="bg-green-500 text-white px-3 py-2 rounded-md hover:bg-green-600">
                                        <i class="fas fa-plus"></i>
                                    </button>
                                @endif
                            </div>
                    ...
                    <div class="flex justify-end space-x-4 mt-8">
                        <a href="{{ route('admin.jurusan.index') }}" class="px-6 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50 transition-colors duration-200">Batal</a>
...
<script>
    function addCompetency() {
        ...
        div.innerHTML = `
            <input type="text" name="competencies[]" required
                class="flex-1 px-4 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
                placeholder="Masukkan kompetensi">
            <button type="button" onclick="removeField(this)" class="bg-red-500 text-white px-3 py-2 rounded-md hover:bg-red-600">
                <i class="fas fa-minus"></i>
    ...
    function addCareer() {
        ...
        div.className = 'flex items-center space-x-2 mb-2';
        div.innerHTML = `
            <input type="text" name="careers[]" required
                class="flex-1 px-4 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
                placeholder="Masukkan prospek karir">
            <button type="button" onclick="removeField(this)" class="bg-red-500 text-white px-3 py-2 rounded-md hover:bg-red-600">
                <i class="fas fa-minus"></i>
